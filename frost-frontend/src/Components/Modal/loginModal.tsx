    ...
    return (
        <div className='ModalSection' onClick={() => {
            ...
            <div className='ModalContainer' onClick={(e: React.MouseEvent<HTMLDivElement>) => {
                ...
                <Input type={'password'} style={{marginBottom: 15}} placeholder={'Пароль'} name={'password'}
                       func={PassSet}/>
                <button className={'forgot'} onClick={() => {
                    modalContext?.updateValue(modalType.forgotPass);
                }}>Забыли пароль?
                </button>
                <button className={"button-blue item-box-item__btn btn-blue-effect button-blue-edited "}
                        onClick={() => {
                            modalContext?.updateValue(modalType.none);
                            SignIn(mail, pass);
                        }} style={{marginTop: 41}}>Войти
                </button>
